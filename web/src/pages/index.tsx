...
export default function Home(props: HomeProps) {
    ...
    try {
      const response = await api.post('/pools', {
        title: poolTitle,
          ...
          alert('Botão criado com sucesso. O código foi copiado para a área de transferência.')    
          setPoolTitle('')    
          } catch (err) {
            console.log(err)
  ...
  return (
    <div className='max-w-[1124px] h-screen mx-auto grid grid-cols-2 gap-28 items-center'>
      <main>
        <Image
          src={logoImg} alt='Logo da aplicação.' />
      ...
      <Image
        src={appPreviewImg} alt="Dois celulares exibindo uma prévia da aplicação"
        quality={100}
      />
    </div>
  )
